...
import { trackAddToCart, trackBeginCheckout } from "@/lib/shopify/tracking";
  ...
  persist(
    (set, get) => ({
      ...
      getCheckoutUrl: () => {
        ...
        if (url) trackBeginCheckout(get().items);
        return url;
      },
...
// Touch imports so tree-shaking keeps the constants referenced (used in tracking + debugging).
void SHOPIFY_STOREFRONT_TOKEN;
void SHOPIFY_STOREFRONT_URL;
