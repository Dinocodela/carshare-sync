...
export default function WrapDetail() {
  ...
  if (!wrap) {
    return (
      <div className="min-h-screen bg-[#F7F2E9] text-[#17211F] flex flex-col">
        <SEO
        ...
        <main className="flex-1 px-6 py-24 text-center">
          <h1 className="font-serif text-4xl">Wrap not found</h1>
          <p className="mt-4 text-[#5C6B67]">
            That wrap doesn’t exist — it may have been renamed.
          </p>
          <Link
            to="/wraps"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#1B6E66] px-6 py-3 text-sm font-medium text-white"
          >
    ...
    <div className="min-h-screen bg-[#F7F2E9] text-[#17211F]">
      ...
      <main className="pb-28 lg:pb-16">
        <div className="max-w-6xl mx-auto px-6 pt-10">
          <Link
            to="/wraps"
            className="inline-flex items-center gap-2 text-sm font-medium text-[#1B6E66] hover:text-[#0E3D3A] transition-colors"
          >
        ...
        <div className="max-w-6xl mx-auto px-6 pt-8 grid gap-10 lg:grid-cols-2">
          <div>
            <WrapImage
              ...
              priority
            />
            <p className="mt-3 text-xs text-[#5C6B67]">
          ...
          <div>
            ...
            <p className="mt-4 text-[#5C6B67] leading-relaxed">
              {wrap.description}
            </p>

            <dl className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div className="rounded-2xl bg-[#FFFDF9] border border-[#E8E1D3] p-4">
                <dt className="text-[11px] uppercase tracking-wider text-[#5C6B67]">
                  Compatibility
                </dt>
              ...
              <div className="rounded-2xl bg-[#FFFDF9] border border-[#E8E1D3] p-4">
                ...
                <dd className="mt-1 tabular-nums text-[#5C6B67]">
            ...
            <a
              ...
              onClick={onDownload}
              className="mt-8 hidden lg:inline-flex items-center gap-2 rounded-full bg-[#1B6E66] px-7 py-3.5 text-sm font-medium text-white shadow-[0_12px_30px_rgba(14,61,58,0.18)] transition-transform motion-safe:hover:-translate-y-0.5"
            >
              <Download className="w-4 h-4" />
              Download free PNG
            </a>

            <p className="mt-4 flex items-start gap-2 text-xs text-[#5C6B67]">
              <Info className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#C6A15B]" />
        ...
        <section className="max-w-6xl mx-auto px-6 mt-16">
          <div className="rounded-3xl bg-[#0E3D3A] p-8 sm:p-11 relative overflow-hidden">
            <div
              aria-hidden
            ...
            <div className="relative">
              ...
              <ol className="mt-7 space-y-4 text-sm text-white/70 list-none p-0">
                {installSteps.map((step, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/10 text-xs text-white tabular-nums">
                      {i + 1}
                    </span>
        ...
        {related.length > 0 && (
          <section className="max-w-6xl mx-auto px-6 mt-16">
            <h2 className="font-serif text-3xl">More wraps</h2>
              ...
              {related.map((r) => (
                <li key={r.slug}>
                  <Link
                    ...
                    className="group block rounded-3xl bg-[#FFFDF9] p-4 shadow-[0_20px_60px_rgba(14,61,58,0.08)] transition-transform motion-safe:hover:-translate-y-1"
                  >
                    <WrapImage
                      ...
                      badge="Concept preview"
                    />
                    <h3 className="mt-4 px-2 font-serif text-xl">{r.title}</h3>
                    <p className="px-2 pb-2 text-xs text-[#5C6B67]">
                      {r.category} · Free
                    </p>
                  </Link>
                </li>
        ...
        <section className="max-w-3xl mx-auto px-6 mt-16">
          <p className="text-xs leading-relaxed text-[#5C6B67]">
            {WRAP_DISCLOSURE}
          </p>
        </section>
      </main>
      ...
      {/* Sticky mobile CTA */}
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 border-t border-[#E8E1D3] bg-[#FFFDF9]/95 backdrop-blur px-4 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
        <a
          href={fileUrl}
          download={wrap.filename}
          onClick={onDownload}
          className="flex w-full items-center justify-center gap-2 rounded-full bg-[#1B6E66] px-6 py-3.5 text-sm font-medium text-white"
        >
          <Download className="w-4 h-4" />
          Download free PNG
        </a>
