...
export function ManageCarAccessDialog({ carId, open, onOpenChange }: ManageCarAccessDialogProps) {
  ...
  const loadAccess = async () => {
    ...
    try {
      ...
      // Fetch profiles for all user_ids in parallel
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            View and manage who can access this car. You can change permissions or revoke access.
          </DialogDescription>
        </DialogHeader>
        ...
        <Card>
          <CardContent className="p-0">
            {loading ? (
              <div className="p-6 text-sm text-muted-foreground">Loading access…</div>
            ) : access.length === 0 ? (
              <div className="p-6 text-sm text-muted-foreground">No shared users yet.</div>
            ) : (
                  ...
                  <li key={row.id} className="p-4 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline">{row.user_id.slice(0, 6)}…</Badge>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="destructive" size="sm" onClick={() => handleRevoke(row)}>
                        Revoke
                      </Button>
                    </div>
                  </li>
