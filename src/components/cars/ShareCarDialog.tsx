...
export function ShareCarDialog({
  carId,
  open,
  onOpenChange,
}: ShareCarDialogProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (!accessToken) {
        toast({
          title: "Not signed in",
          description: "You must be signed in to share access.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (error) {
        ...
        const status =
          (error as any)?.context?.responseStatus ?? (error as any)?.status;
        ...
        const description =
          ...
          (status === 404
            ...
            : status === 403
            ? "Only the car owner can share access."
            : status === 401
            ? "You must be signed in to share access."
            : fallback);
      ...
      toast({
        ...
        description: `Shared with ${email} as ${permission}.`,
      });
      reset();
      onOpenChange(false);
    } catch (e: any) {
  ...
  return isMobile ? (
    ...
    >
      ...
      >
        {/* Custom header: title/desc on the left, close on the right */}
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Grant another client access to this car&apos;s data.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="editor">
                  Editor (can modify fixed expenses)
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="pt-2 flex justify-end gap-2">
            ...
            <Button onClick={handleShare} disabled={loading || !carId}>
