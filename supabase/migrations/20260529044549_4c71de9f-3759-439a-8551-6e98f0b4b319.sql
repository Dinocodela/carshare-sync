CREATE OR REPLACE FUNCTION public.get_clients_for_hosts()
 RETURNS TABLE(user_id uuid, first_name text, last_name text, company_name text, email text, phone text, created_at timestamp with time zone, has_cars boolean, car_count bigint, hosted_car_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  -- Verify the requesting user is a host (role lives in user_roles now)
  IF NOT public.has_role(auth.uid(), 'host') THEN
  ...
  FROM profiles p
  WHERE EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = p.user_id AND ur.role = 'client')
